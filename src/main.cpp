...
void handleAP()
{
  ...
  if (debug)
    uart.print("new device ip:"), uart.println(ip.toString());
...
void loadInit()
{
  ...
  if (strncmp(dbg.c_str(), "full", 4) == 0)
    fileDump = debug = true;
  else if (strncmp(dbg.c_str(), "debug", 4) == 0)
    fileDump = false, debug = true;
  else
    fileDump = debug = false;
  uart.end();
  delay(500);
...
void apMode()
{
  ...
  WiFi.mode(WIFI_AP);
  if (debug)
    uart.println("Setting up in AP Mode");
  isAP = true;
  ...
  bool openAP = false;
  if (strncmp(key.c_str(), "", 1) == 0 || keyLength < 8)
    openAP = true;
  ...
  if (openAP)
  {
    ...
    if (debug)
      uart.println(">dbg.WiFiMode(Open AP)");
  }
  else
  {
    WiFi.softAP(ssid.c_str(), key.c_str());
    if (debug)
      uart.println(">dbg.WiFiMode(Safe AP)");
  }
...
void rawClientConnect()
{
  ...
  {
    if (debug)
      uart.println("connection failed");
  }
  else if (debug)
...
void setupProtocol()
{
  if (clientMode)
  {
    if (isAP && mqtt)
    {
      mqtt = false, ws = true;
      if (debug)
        uart.println("MQTT and AP mode won't work well, changing to WS");
    }
    if (ws)
    {
    ...
    {
      if (debug)
        uart.println(">dbg.protocol(MQTT)");
...
void loadProtocol()
{
  ...
  if (strncmp(protocol.c_str(), "raw", 2) == 0)
    ws = false, rawTCP = true, mqtt = false;
  else if (strncmp(protocol.c_str(), "ws", 2) == 0)
    ws = true, rawTCP = false, mqtt = false;
  else if (strncmp(protocol.c_str(), "mqtt", 2) == 0)
    ws = false, rawTCP = false, mqtt = true;
...
void loadNames()
{
  ...
  if (fileDump)
  {
    uart.print("Subscription topic:");
    uart.println(subTopic);
    uart.print("Publishing topic:");
    uart.println(pubTopic);
    uart.print("Name:");
    uart.println(name);
  }
}
...
void wsClientEvent(WStype_t type, uint8_t *payload, size_t length)
{
  ...
  {
    if (debug)
      uart.printf("[WSc] Disconnected!\n");
  }
  else if (type == WStype_CONNECTED)
  {
    if (debug)
      uart.printf("[WSc] Connected to url: %s\n", payload);
  }
}
...
void rawServer_handleClients()
{
  ...
  if (rawServer.hasClient())
  {
    for (i = 0; i < NET_MAX_CLIENTS; i++)
    {
      ...
      {
        if (rawServer_clients[i])
          rawServer_clients[i].stop();
  ...
  for (i = 0; i < NET_MAX_CLIENTS; i++)
  {
    if (rawServer_clients[i] && rawServer_clients[i].connected())
    {
      if (rawServer_clients[i].available())
      {
        ...
        while (rawServer_clients[i].available())
          uart.write(rawServer_clients[i].read());
      }
    }
  }
  // check UART for data
