...
namespace Mem
{
  ...
  String readString(int addr, int len)
  {
    String tmp = "";
    for (int i = 0; i < len; i++)
      tmp += (char)EEPROMr.read(addr + i);
    return tmp;
  }
  void writeString(const String &str, int addr, int len)
  {
    for (int i = 0; i < len; i++)
      EEPROMr.write(addr + i, str[i]);
  }
  ...
  {
    for (int i = 0; i <= 3; i++)
      EEPROMr.write(addr + i, (ip >> 8 * (3 - i)) & 0xff); // MSB first
  }

  void writeIP(IPAddress ip, int addr)
  {
    for (int i = 0; i <= 3; i++)
      EEPROMr.write(addr + i, ip[i]); // MSB first
  }
  ...
  {
    for (int i = 0; i <= 3; i++)
      EEPROMr.write(addr + i, (num >> 8 * (3 - i)) & 0xff); // MSB first
  }
  int readInitCode()
  {
